        ...
        if password == MASTER_PASSWORD:
            try:
                with open(LICENSE_FILE, "wb") as f:
                    pickle.dump({"licensed": True, "activated": datetime.now().isoformat()}, f)
                self.licensed = True
                return True
            except Exception as e:
...
DISPLAY_WIDTH    = 960
DISPLAY_HEIGHT   = 600
DISPLAY_DELAY_MS = 33   # ~30 fps display tick

TARGET_FPS = 20

# How many pixels need to be non-zero for a frame to be considered "good"
# This filters out black/corrupt frames from being saved as last_good_frame
...
# Latest live frame — updated every frame during normal streaming
latest_frame = None
frame_lock   = threading.Lock()

# Last good frame — only used when stream times out or disconnects
# A "good" frame = fully decoded, not black/corrupt, has real content
last_good_frame      = None
last_good_frame_lock = threading.Lock()

# Flag: True only when stream has actually timed out or disconnected
# NOT set during normal streaming — live frames always show live
stream_lost      = False
...
def is_good_frame(img):
    """
    Returns True only if the frame looks like real content:
    - Has the right number of dimensions
    - Not mostly black/corrupt (enough non-zero pixels)
    This is what we save as last_good_frame.
    """
    if img is None: return False
    if len(img.shape) != 3: return False
    if img.shape[0] < 10 or img.shape[1] < 10: return False
    # Count non-zero pixels — a corrupt or black frame will fail this
...
async def handle_client(ws, path=None):
    ...
    try:
        async for msg in ws:
            try:
                ...
                if t == "register":
                    ...
                    # Client reconnected — clear stream_lost so live frames show again
                    ...
                    async def on_track(track):
                        ...
                        # Stream is live — clear lost flag
                        with stream_lost_lock:
                            stream_lost = False
                        ...
                        try:
                            while True:
                                try:
                                    # Normal frame received — show it live
                                    frame = await asyncio.wait_for(track.recv(), timeout=5.0)

                                    img = frame.to_ndarray(format="bgr24")

                                    # Only save as last_good_frame if it passes quality check
                                    # This happens on every clean frame during normal streaming
                                    if is_good_frame(img):
                                        with last_good_frame_lock:
                                            global last_good_frame
                                            last_good_frame = img.copy()

                                    # Always show the live frame — never delay or buffer
                                    ...
                                    # Frame timed out — switch to last good frame so
                                    # user sees clean freeze instead of broken/glitchy frame
                                    with stream_lost_lock:
                                        stream_lost = True
                            ...
                            # Track ended (disconnect) — show last good frame
                            with stream_lost_lock:
                                stream_lost = True
        ...
        # WebSocket disconnected — show last good frame
        with stream_lost_lock:
            stream_lost = True
        with streams_lock:
...
class VideoDisplay(tk.Canvas):
    ...
    def _tick(self):
        try:
            ...
            # - stream_lost=False → always show latest live frame (normal streaming)
            # - stream_lost=True  → show last_good_frame so user sees clean freeze
            with stream_lost_lock:
                lost = stream_lost

            if lost:
                # Connection lost / timeout — show last good frame (clean freeze)
                with last_good_frame_lock:
                    frame = last_good_frame
            else:
                # Normal streaming — show latest live frame directly
                with frame_lock:
                    frame = latest_frame
            ...
            if frame is not None:
                try:
                    ...
                    # Show FPS only during live streaming
                    if not lost:
                        self.create_text(10, dh - 20, text=f"FPS: {fps_display}",
                                         fill="#00FF00", font=("Arial", 10), anchor="w")
                    else:
                        # Small indicator that stream is paused — not alarming
